...
from . import layers, normalization, utils, attention
...
class UNetModel(nn.Module):
  ...
  def __init__(self, config):
    ...
    ## initial.
    if self.init_self:  # .freeze qkv and alpha, tune others.
      ...
      for layer in [self.q, self.k, self.v]:
        for p in layer.parameters():
          ...
          if layer is self.v:
            ...
            p.detach().zero_()
    else:  # tune QK and weight, freeze others.
      for name, layer in self.named_parameters():
        if name in ['alpha', 'q.weight', 'k.weight']:
          layer.requires_grad=True
        else:
          layer.requires_grad=False
      self.v.weight.detach().copy_(torch.eye(*self.v.weight.shape))

  def forward(self, x, time_cond, text_emb=None, esm_contact=None):
  
    shapes = text_emb.shape[1]

    if esm_contact is not None:
        p = torch.arange(0, text_emb.shape[1]).cuda()
        p = torch.abs(p[None,:] - p[:,None])
        gaussian_probs = distance_to_gaussian(p, 2.0)
        bs_gaussian_prbs = gaussian_probs.unsqueeze(0).expand(text_emb.shape[0],-1,-1)
        diagonal_masks = torch.where(esm_contact == 0, torch.tensor(0, dtype=torch.float).cuda(), torch.tensor(1, dtype=torch.float).cuda())
        ...
        weight =  0.7*esm_contact + 0.3*bs_gaussian_prbs
        weight = weight / (weight.sum(-1, keepdim=True) + torch.full_like(weight, 1e-9).cuda())
        if self.alpha.requires_grad is True:
