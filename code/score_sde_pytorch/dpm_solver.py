...
def model_wrapper(
    ...
    def get_model_input_time(t_continuous):
        ...
        if noise_schedule.schedule == 'discrete':
            return (t_continuous - 1. / noise_schedule.total_N) * 2000.
        else:
            return t_continuous
    ...
    def noise_pred_fn(x, t_continuous, cond=None):
        ...
        if model_type == "noise":
            ...
            return -expand_dims(sigma_t, x.dim()) * output  # socre是噪声除以了std, 所以需要还原为output.
...
class DPM_Solver:
    ...
    def sample(self, x, conditional_mask, x_initial, steps=20, t_start=None, t_end=None, order=2, skip_type='time_uniform',
        method='multistep', lower_order_final=True, denoise_to_zero=False, solver_type='dpmsolver',
        atol=0.0078, rtol=0.05, return_intermediate=False,
    ):
        """
        ...
        with torch.no_grad():
            if method == 'adaptive':
                ...
                with tqdm(total=steps, desc='sampling') as pbar:
                ...
                for step, order in tqdm(enumerate(orders), desc='sampling'):
