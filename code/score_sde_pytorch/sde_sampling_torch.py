...
class AdaptivePredictor(Predictor):
  def __init__(self, sde, score_fn, shape, eps=1e-3, 
    ...
    self.safety = safety
    self.n = shape[1]*shape[2]*shape[3]
    self.exp = exp
  ...
  def update_fn(self, x, t, h, x_prev, context, esm_contact): 
    ...
    z = torch.randn_like(x)
    drift, diffusion = my_rsde(x, t, context, esm_contact)
    ...
    drift_Heun, diffusion_Heun = my_rsde(x + K1, t - h, context, esm_contact)
...
def shared_predictor_update_fn(x, t, h, context, esm_contact, sde, model, predictor, x_prev=None, shape=None,
    eps=1e-3, abstol = 1e-2, reltol = 1e-2, safety = .9, exp=0.9):
  """A wrapper that configures and returns the update function of predictors."""
  score_fn = get_score_fn(sde, model, train=False)
  predictor_obj = predictor(sde, score_fn, shape=shape, eps=eps, 
    abstol = abstol, reltol = reltol, safety = safety, exp=0.9)
  return predictor_obj.update_fn(x, t, h, x_prev, context, esm_contact)


def get_pc_sampler(sde, shape, predictor, corrector=None, snr=None,
                   n_steps=1, probability_flow=False,
                   denoise=True, eps=1e-3, device='cuda',
                   abstol = 1e-2, reltol = 1e-2, safety = .9, exp=0.9, adaptive=False, h_init = 1e-2):
  ...
  def pc_sampler_adaptive(model, condition=None, context=None, esm_contact=None, infer=False):
    ...
    with torch.no_grad():
      ...
      while (torch.abs(t - eps) > 1e-6).any():
        start = time.time()
        x, x_prev, t, h = predictor_update_fn(x, t, h, x_prev=x_prev, model=model, context=context, esm_contact=esm_contact)
        N = N + 1
        end = time.time()
        print(f"now, {N-1}: {torch.abs(t - eps).max()}, times:{end-start}")
      ...
      if denoise:
        ...
        x = x + (std[:, None, None, None] ** 2) * model(x, eps_t, context, esm_contact) 
      return x, N + 1
